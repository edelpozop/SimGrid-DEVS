...
int toManual(int argc, char * argv[]) {
  ...
  while (1) {
    ...
    } else if (strcmp(MSG_task_get_name(task), "toManual") == 0) {
      printf("\033[0m");
...
int manual(int argc, char * argv[]) {
  ...
  while (1) {
    ...
    if (strcmp(MSG_task_get_name(task), "Finalize") == 0) {
    ...
    } else if (strcmp(MSG_task_get_name(task), "Manual") == 0) {
      printf("Turned off - Time: %g\n", time);
      int sleep = rand() % 100;
      ...
      MSG_task_destroy(task);
      task = NULL;
      new_task = MSG_task_create("Automatic", 0, 0, NULL);
...
int toAutomatic(int argc, char * argv[]) {
  ...
  while (1) {
    ...
    } else if (strcmp(MSG_task_get_name(task), "Automatic") == 0) {
...
int controller(int argc, char * argv[]) {
  ...
  while (1) {
    ...
    sprintf(light, "toManual");
    MSG_task_send(task, light);

    task = MSG_task_create("Finalize", 0, 0, NULL);

    sprintf(light, "manual");
    MSG_task_send(task, light);

    task = MSG_task_create("Finalize", 0, 0, NULL);

    sprintf(light, "toAutomatic");
    MSG_task_send(task, light);
