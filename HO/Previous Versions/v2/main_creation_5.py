...
def main():
    ...
    with open("main.c", "w+") as main:
        ...
        main.write('#define WIDTH ' + str(width - 1) + '\n')
        main.write('#define DEPTH ' + str(depth - 1) + '\n\n')
    ...
    with open("sim.c", "w+") as sim:
        ...
        sim.write('#define WIDTH ' + str(width - 1) + '\n')
        sim.write('#define DEPTH ' + str(depth - 1) + '\n\n')
        ...
        sim.write('\tmsg_host_t host = MSG_host_by_name(hostN);\n\n')
        sim.write('\twhile(1)\n\t{\n')
        ...
        sim.write('\t\t\tif( i == DEPTH - 1 )\n\t\t\t{\n')
        sim.write('\t\t\t\tnew_task = MSG_task_create("0", 0, 0, NULL);\n')
        sim.write('\t\t\t\tsprintf(dest, "cp-%d-0", i);\n')
        sim.write('\t\t\t\tMSG_task_send(new_task, dest);\n')
        sim.write('\t\t\t\tbreak;\n')
        sim.write('\t\t\t}\n\t\t\telse\n\t\t\t{\n')
        sim.write('\t\t\t\tnew_task = MSG_task_create("0", 0, 0, NULL);\n')
        ...
        sim.write('\tconst char* hostN[30], dest[30];\n')
        sim.write('\tsprintf(hostN, "%s", argv[2]);\n')
